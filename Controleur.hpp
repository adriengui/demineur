...
class Controleur {
    ...
    public :
        Controleur(Modele *m, Vue *v) : modele_(m), vue_(v) {
			...
			vue_->getAppliRef().attach(vue_->getGrilleRef(), 0,1,0,1);
			
			vue_->getBoutonRetourRef()=Gtk::Button("retour");
			vue_->getBoutonRetourRef().signal_clicked().connect(sigc::mem_fun(*this, &Controleur::afficheAccueil));

			vue_->getAppliRef().attach(vue_->getBoutonRetourRef(), 1,2,0,1, Gtk::SHRINK, Gtk::SHRINK);
