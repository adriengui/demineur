...
public :
  ...
  void afficheJeu() {
	...
	//vue_->getBoutonsGrilleRef()[x*modele_->getColonnes()+y].signal_button_press_event().connect(sigc::bind<GdkEventButton*, int, int>(sigc::mem_fun(*this, &Controleur::setFlag), x, y));
	auto& button = vue_->getBoutonsGrilleRef()[x*modele_->getColonnes()+y];
	button.signal_button_press_event().connect([this, x, y](GdkEventButton* event) {return this->setFlag(event,x,y);});
      }
    }
    for(int x=0;x<modele_->getLignes();x++) {
      for(int y=0;y<modele_->getColonnes();y++) {
  ...
  int setFlag(GdkEventButton* event, const int x, const int y) {
  ...
  void fillGrille() {
	...
	if(modele_->getCell(x,y).getOpen()) {
	  ...
	  if(n!=0) {
	    ...
	    vue_->getBoutonsGrilleRef()[x*c+y].set_label("");
	  }
	} else if(modele_->getCell(x,y).getState() == flag){
	  vue_->getBoutonsGrilleRef()[x*c+y].set_label("F");
	} else {
	  vue_->getBoutonsGrilleRef()[x*c+y].set_label("");
	}
      }
